...
function calcBefore(jigyoShotoku, aoshiroKey, age, deps) {
  ...
  const nhi          = calcTokyoNHI(shotokuAfter, age); // 早見表と同じ東京都料率
...
function FreelancePage() {
  ...
  return (<div>
    ...
    <div style={G.card}>
      ...
      <RRow label="国民健康保険料" sub="(概算・全国平均)" value={r.nhi} monthly/>
...
function SimPage({ onCalc }) {
  ...
  return (<div>
    ...
    <p style={{fontSize:11,color:"#334155",textAlign:"center",lineHeight:1.9}}>
      ※国民健康保険は全国平均概算。実際は市区町村により異なります<br/>
  ...
  return (
    <div>
      ...
      <div style={G.card}>
        ...
        {/* ■ 経費算入の税効果 */}
        <Block title="■ 経費になる場合の追加節税効果" color="#a78bfa">
          <div style={{background:"rgba(167,139,250,0.08)",border:"1px solid #4c1d95",borderRadius:10,padding:"12px"}}>
            <div style={{display:"grid",gridTemplateColumns:"1fr auto 1fr",alignItems:"center",gap:8,textAlign:"center",marginBottom:10}}>
              <div>
                <div style={{fontSize:10,color:"#64748b"}}>実質会員費</div>
                <div style={{fontSize:16,fontWeight:700,color:"#f87171"}}>¥{fmt(netFeeCost)}</div>
              </div>
              <div style={{fontSize:14,color:"#475569"}}>×</div>
              <div>
                <div style={{fontSize:10,color:"#64748b"}}>限界税率（所得税+住民税）</div>
                <div style={{fontSize:16,fontWeight:700,color:"#a78bfa"}}>{taxRate}%</div>
              </div>
            </div>
            <div style={{textAlign:"center",borderTop:"1px solid #334155",paddingTop:8}}>
              <div style={{fontSize:11,color:"#94a3b8"}}>経費算入による節税効果</div>
              <div style={{fontSize:22,fontWeight:700,color:"#a78bfa"}}>約¥{fmt(taxEffect)}</div>
              <div style={{fontSize:10,color:"#64748b",marginTop:2}}>→ 税負担増をさらに圧縮できる</div>
            </div>
          </div>
        </Block>
        ...
        <Block title="■ 全体整理" color="#38bdf8">
          {[
            ...
            {label:"経費算入の節税効果", val:`+¥${fmt(taxEffect)}`, color:"#a78bfa"},
          ].map(({label,val,color})=>(
...
// ─── 概算早見表ページ ─────────────────────────────
// 東京都 令和8年度 国保料率（40歳未満）
// (所得 - 43万) × 10.4% + 64,096 ≒ スプレッドシートの実績値と一致
function calcTokyoNHI(income, age) {
  const base = Math.max(income - 430000, 0);
  const iryo  = Math.min(base * 0.0771 + 47300, 650000);
  const shien = Math.min(base * 0.0243 + 14700, 220000);
  const kaigo = age === "40〜64歳" ? Math.min(base * 0.02 + 16100, 170000) : 0;
  return Math.floor(iryo + shien + kaigo);
}
  ...
  return (
    <div>
      ...
      <div style={{fontSize:11,color:"#64748b",marginBottom:10,textAlign:"center"}}>
        ...
        {pref !== "東京" && <span style={{color:"#f59e0b"}}>※現在のバージョンでは国保は東京都料率を使用しています</span>}
      </div>
      ...
      <div style={{maxWidth:480,margin:"0 auto"}}>
        ...
        <div style={{
          ...
          <div style={{textAlign:"right",flexShrink:0,marginLeft:12}}>
            <div style={{
              fontSize:9, color:"#c9a84c", letterSpacing:"0.1em",
              marginBottom:2, opacity:0.8,
            }}>Provided by</div>
            <div style={{
            ...
            <div style={{
              fontSize:8, color:"#475569", marginTop:2, lineHeight:1.4,
            }}>一般社団法人<br/>中小企業財務支援機構</div>
          </div>
        </div>
